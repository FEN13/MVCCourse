...
namespace ExifReader
{
	public sealed class ExifTagCollection : IEnumerable<ExifTag>
	{
		...
		private void ReadTags(PropertyItem[] pitems)
		{
			...
			foreach (PropertyItem pitem in pitems)
			{
				...
				if (pitem.Type == 0x1)
				{
					...
					#endregion BYTE (8-bit unsigned int)
				}
				else if (pitem.Type == 0x2)
				{
					...
					#endregion ASCII (8 bit ASCII code)
				}
				else if (pitem.Type == 0x3)
				{
					...
					#endregion 3 = SHORT (16-bit unsigned int)
				}
				else if (pitem.Type == 0x4)
				{
					...
					#endregion 4 = LONG (32-bit unsigned int)
				}
				else if (pitem.Type == 0x5)
				{
					...
					#endregion 5 = RATIONAL (Two LONGs, unsigned)
				}
				...
				else if (pitem.Type == 0xA)
				{
					...
					#endregion 10 = SRATIONAL (Two SLONGs, signed)
				}
		...
		{
			string[] vals = new[] { "", "Y", "Cb", "Cr", "R", "G", "B" };

			return bytes.Aggregate("", (current, b) => current + vals[b]);
		}
		...
		#endregion IEnumerable<ExifTag> Members
